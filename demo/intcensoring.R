...
for(i in 1:50){
  ...
  #####################################EM Algorithm##############################
  #####---------------------------------------------------------------###########
  EM = fpiter(par = pvec, fixptfn = intEM,
                             objfn = loglik, A = A,
  ...
  #####---------------------------------------------------------------###########
  pureR = SQUAREM::squarem(par = pvec, fixptfn = intEM,
                              objfn = loglik, A = A,
                              control = list(tol = 1e-8))
  pureR

  #####---------------------------------------------------------------###########
  ###############Squarem to accelerate EM Algorithm (Rcpp)#######################
  #####---------------------------------------------------------------###########
  Rcpp = RcppSqEM::squarem(par = pvec, fixptfn = intEM,
                              objfn = loglik, A = A,
                              control = list(tol = 1e-8))
  Rcpp
}
